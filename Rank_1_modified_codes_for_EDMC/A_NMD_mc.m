function [Theta,err,i,time]=A_NMD_mc(X,r,param)
...
% that is a simple approach which alternates between the optimal feasible Z satisfing P_{Omega}(Z)=P_{Omega}(X) and P_{Omega^C}(Z)<=0;
% and one optimal Theta that is the TSVD of Z, Theta=TSVD(Z-dee^T,r).
%
% The extrapolation parameter is adaptively chosen accordingly to the decrease of the residual at each iteration.
%
%****** Input ******
...
for i=1:param.maxit
    ...
    if param.observation==1
        back(i)=norm(Z-Theta+param.d,'fro')/normX;
    elseif param.observation==2
        back(i)=norm(Z+Theta-param.d,'fro')/normX;
    end
