function [Theta,err,i,time]=BCD_NMD_mc(X,r,param)
...
% 
% using block coordinate descent algorithm; 
% 
% Update of Z: Z^{k+1}=argmin_Z || Z - W^k H^k ||_F^2  s.t.  max(0,Z)=X
% 
% Update of W: W^{k+1}=argmin_W || Z^{k+1} - W H^k ||_F^2, 
%
% Update of H: H^{k+1}=argmin_H || Z^{k+1} - W^{k+1} H ||_F^2
%
%****** Input ******
...
Z=param.Z0; W=param.W0; H=param.H0; Theta=W*H; 
...
if param.display == 1
    disp('Running BCD-NMD, evolution of [iteration number : relative error in %]');
end
